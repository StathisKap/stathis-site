...
{#if is_subtitle_done}
	<div in:fade class="pt-12 text-lg">
		...
		<p class="text-xl text-slate-500 text-center">
			...
			It is being developed with Svelte, Typescript, and Tailwind. It is hosted as a node server
			within an LXC container on the ProxMox Type 1 Hypervisor, running on an old laptop in my house in Greece.
			The site is accessible through a cloudflare argos tunnel, using the `cloudflared` deamon on a different 
			LXC container. The domain name is a free one from freenom. So this entire site is 100% from scrath and 100% free. (except power bill ofc)
		</p>
	</div>
{/if}
